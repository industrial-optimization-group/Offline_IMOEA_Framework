...
def plot_refv(objs, preference, iteration, ideal, nadir, path):
    len_data = np.shape(objs)[0]
    ...
    if np.shape(objs)[0] > 0:
        objs_col = np.zeros(len_data).reshape(-1, 1)
...
#                          (1.0, 'white')]
    ...
    plot(fig, filename=path+"/reference_vectors_"+str(iteration)+".html")
    print('Plotting done!!')
