...
def build_surrogates(problem_testbench, problem_name, nobjs, nvars, nsamples, is_data, x_data, y_data, surrogate_type, Z=None, z_samples=None, nd_points=None):
    ...
    if surrogate_type == "generic_fullgp":
        ...
        problem.train(htgp, model_parameters=nd_points)
    else:
...
def run_optimizer_htgp(problem_testbench, problem_name, nobjs, nvars, sampling, nsamples, is_data, surrogate_type, run):
    ...
    if np.shape(y)[0] > 1:
        non_dom_front = ndx(y)
        nd_points = non_dom_front[0][0]
    else:        
        nd_points = None
    
    print("Building trees...")
    surrogate_problem, time_taken = build_surrogates(problem_testbench,problem_name, nobjs, nvars, nsamples, is_data, x, y, "htgp", nd_points={'nd_points':nd_points})
    print("Building GPs...")
    total_points_all = 0
    ...
    evolver_opt_tree = RVEA(surrogate_problem, use_surrogates=True, n_iterations=50, n_gen_per_iter=30)
