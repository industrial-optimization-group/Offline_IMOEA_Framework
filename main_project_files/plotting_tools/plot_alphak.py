...
def plot_vals_k(objs, unc, reference_vectors, preference, iteration, closest_point,  interaction_count, ideal, nadir):
    objs_orig = objs
    ...
    closest_pnt = pd.DataFrame(np.hstack((closest_point.reshape(1,-1), [[2.5]])), columns=columns + ["color"])
    data_final = pd.concat([objs, closest_pnt])
    colrefv = (np.ones(reference_vectors.shape[0])*2).reshape(-1,1)
    ref_v = pd.DataFrame(np.hstack((reference_vectors, colrefv)), columns=columns + ["color"])
    data_final = pd.concat([objs, ref_v])
    if preference is not None:
        pref = pd.DataFrame(np.hstack((preference.reshape(1,-1), [[2.7]])), columns=columns + ["color"])
        data_final = pd.concat([objs, pref])
    else:
        data_final = objs
    frames = [objs, closest_pnt, ref_v, pref]
...
#                          (1.0, 'white')]
    ...
    color_scale_custom = [(0.0, 'rgb(69,2,86)'), (0.083, 'rgb(59,28,140)'), (0.167, 'rgb(33,144,141)'),
                          ...
                          (0.334, 'magenta'), (0.7, 'magenta'),
                          (0.7, 'red'), (0.85, 'red'), 
                          (0.85, 'goldenrod'), (0.95, 'goldenrod'),
                          (0.95, 'white'),(1.0, 'white')]
    ...
    fig = ex.scatter_3d(data_final, x='f_1', y='f_2', z='f_3',
                    color='color',  color_continuous_scale=color_scale_custom, range_color=(0,3))
    plot(fig, filename="alpha_k_"+str(iteration)+"_"+str(interaction_count)+".html")
    print('Plotting done!!')
