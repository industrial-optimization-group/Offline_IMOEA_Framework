...
def Prob_APD_select_v1(
    ...
    for i in range(0, len(vectors.values)):
        ...
        if len(sub_population_fitness > 0):
            ...
            """
            apd_mean = np.mean(apd, axis=2).flatten()
            apd_sd = np.std(apd, axis=2).flatten()
            apd_objs_old = np.vstack((apd_mean,apd_sd)).transpose()
            non_dom_front = nd2(apd_objs_old)
            apd_objs = apd_objs_old[non_dom_front]
            
            if len(apd_objs)>2:
                kneedle = KneeLocator(apd_objs[:, 0], apd_objs[:, 1], curve='convex', direction='decreasing')
                # plt.scatter(apd_objs[:, 0], apd_objs[:, 1])
                knee_pos = kneedle.knee_x
                if knee_pos is not None:
                    #kneedle.plot_knee()
                    #plt.show()
                    knee_pos = kneedle.knee_x
                    minidx = np.where(apd_objs_old[:,0] == apd_objs[knee_pos-1,0])
                else:
                    minidx = np.where(apd_objs_old[:, 0] == np.nanmin(apd_objs[:, 0]))
            ...
            #    (1 + np.dot(penalty_factor, np.mean(angles, axis=2))),
            #)
            #apd = np.mean(sub_popfm,axis=2)
            #rank_apd = apd
            #print("Rank APD:")
            #print(rank_apd)
            #rank_apd = np.around(rank_apd,3)
    ...
    for i in range(len(assigned_vectors)-1):
        ax.annotate(assigned_vectors[i], ((translated_fitness[i, 0]+0.03), (translated_fitness[i, 1]+0.08)))
    for i in vector_anno:
        ax.annotate(vector_anno[i], ((vectors.values[i, 0]+0.01), (vectors.values[i, 1]+0.01)))
