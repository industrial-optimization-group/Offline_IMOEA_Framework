function [Output,Boundary,Coding] = P_objective_v0(Operation,Problem,M,Input)
    ...
    switch Problem(1:k)
        ...
        case 'C1_DTLZ'
            [Output,Boundary,Coding] = P_C1_DTLZ(Operation,Problem,M,Input);
        case 'C2_DTLZ'
            [Output,Boundary,Coding] = P_C2_DTLZ(Operation,Problem,M,Input);
        case 'C3_DTLZ'
            [Output,Boundary,Coding] = P_C3_DTLZ(Operation,Problem,M,Input);
        case 'WFG'
        ...
        case 'Others'
             [Output,Boundary,Coding] = P_others(Operation,Problem,Input);
        otherwise
            error(['????',Problem,'??????.']);
    end
end
%% Added by Tinkle Chugh
function [Output,Boundary,Coding] = P_others(Operation,Problem,Input)
    persistent K;
...
%             K = [30 30 30 10 11 10];
            ...
            D = K;
            if strcmp(Problem,'Others2')
                    MaxValue = [6*sin(pi/12)+2*pi*cos(pi/12),6*cos(pi/12)];
                    MinValue = [6*sin(pi/12),-2*pi*sin(pi/12)];
            elseif strcmp(Problem,'Others3')
                    MaxValue = [pi,5,5];
                    MinValue = [-pi,-5,-5];
            elseif strcmp(Problem,'Others4')
                    MaxValue = [2,2];
                    MinValue = [-2,-2];
            elseif strcmp(Problem,'Others5')
                    MaxValue = [3,3];
                    MinValue = [-3,-3];
            elseif strcmp(Problem,'Others1')
                    MaxValue = 3*ones(1,D);
                    MinValue = zeros(1,D);
            elseif strcmp(Problem,'Others6')
                    MaxValue = [0.3000,0.7775, 0.0990, 0.0770,1.2000,0];
                    MinValue = [0.2792,0.6607,0.0720,0.0560,0.6631,-0.2859];
...
%             end
        ...
        case 'value'
            ...
            switch Problem
                case 'Others1' % this KNO1 problem
                    c = Population(:,1)+Population(:,2);
                    r = 20-( 11+3*sin((5*c).*(0.5*c)) + 3*sin(4*c) + 5 *sin(2*c+2));
                    g = (pi/2.0)*(Population(:,1)-Population(:,2)+3.0)/6.0;
    
                    FunctionValue(:,1)= 20-(r.*cos(g));
                    FunctionValue(:,2)= 20-(r.*sin(g));
...
%                     r = 9 - (C + D+ E);
%                     phi = pi./(12*(x1-x2+3));
%                     FunctionValue(:,1) = 20 - r.*cos(phi);
%                     FunctionValue(:,2) = 20 - r.*sin(phi);
                case 'Others2' % this is OKA1 problem
                    x1 = cos(pi/12).*Population(:,1) - sin(pi/12).*Population(:,2);
                    x2 = sin(pi/12).*Population(:,1) + cos(pi/12).*Population(:,2);
                    FunctionValue(:,1) = x1;
                    FunctionValue(:,2) = sqrt(2*pi) - sqrt(abs(x1)) + 2*(abs(x2-3*cos(x1)-3)).^0.5;
                case 'Others3' % this is OKA2 problem
                     FunctionValue(:,1) = Population(:,1);
                     FunctionValue(:,2) = 1 - (1/(4*pi*pi)).*(Population(:,1)+pi).^2 + (abs(Population(:,2)-5*cos(Population(:,1)))).^(1/3) ...
                         + (abs(Population(:,3)-5*sin(Population(:,1)))).^(1/3);
                case 'Others4' % this is VLMOP2
                    sum3 = 0;
                    for i=1:D
                        sum3 = sum3 + (Population(:,i) - 1/sqrt(D)).^2;
                    end
                    sum4 = 0;
                    for i=1:D
                        sum4 = sum4 + (Population(:,i) + 1/sqrt(D)).^2;
                    end
                    FunctionValue(:,1) = 1 - exp(-sum3);
                    FunctionValue(:,2) = 1 - exp(-sum4);
                 case 'Others5' % this is VLMOP3
                     FunctionValue(:,1) = 0.5*(Population(:,1).^2 + Population(:,2).^2) + sin(Population(:,1).^2 + Population(:,2).^2);
                     FunctionValue(:,2) = (3*Population(:,1)-2*Population(:,2)+4).^2/8 + (Population(:,1)-Population(:,2)+1).^2/27 + 15;
                     FunctionValue(:,3) = 1./(Population(:,1).^2+Population(:,2).^2+1)-1.1*exp(-Population(:,1).^2-Population(:,2).^2);
                     
                case 'Others6' % this is airfoil problem
                    for i=1:size(Population,1)
                        FunctionValue(i,:) = airfoil(Population(i,:));
                    end
            end
%             FunctionValue(:,2) = g.*h;
        ...
        case 'true'
            switch Problem
                case {'Others1'}
                    Population = dlmread('Real_PF_KNO1.txt');
                case {'Others2'}
                    Population(:,1) = linspace(0,2*pi,500);
                    Population(:,2) = sqrt(2*pi) - sqrt(Population(:,1));
                case {'Others3'}
                    Population(:,1) = linspace(-pi,pi,500);
                    Population(:,2) = 1 - (1/(4*pi*pi))*(Population(:,1)+pi).^2;
                case {'Others4'}
                    Population = dlmread('PF_VLMOP2.txt');
        ...
        case 'value'
            ...
            switch Problem
                case 'DTLZ1'
                    ...
                    %g = (K+sum((Population(:,M:end)-0.5).^2-cos(20.*pi.*(Population(:,M:end)-0.5)),2));
%                     
%g = 100*(K+sum((Population(:,M:end)-0.5).^2-cos(2.*pi.*(Population(:,M:end)-0.5)),2));
                ...
                case 'DTLZ7'
                    g    = 1+(9/6)*mean(Population(:,M:end),2);
...
function [Output,Boundary,Coding] = P_C1_DTLZ(Operation,Problem,M,Input)
    persistent K;
    Boundary = NaN; Coding = NaN;
    switch Operation
        %????????????
        ...
        case 'value'
            ...
            cv = 1 - FunctionValue(:,M)/0.6 - sum(FunctionValue(:,1:M - 1), 2)/0.5;
            cv(cv > 0) = 0;
            cv(cv < 0) = -cv(cv < 0) ;
            Output = [FunctionValue, cv];
        %????????????
        case 'true'
            switch Problem
                case 'C1_DTLZ1'
...
function [Output,Boundary,Coding] = P_C2_DTLZ(Operation,Problem,M,Input)
    persistent K;
    persistent r;
    Boundary = NaN; Coding = NaN;
    switch Operation
        ...
        case 'value'
            ...
            sum1 = sum(FunctionValue.^2,2);
            min1 = 1e200*ones(size(FunctionValue,1), 1);
            for i = 1:M
                temp1 = (FunctionValue(:,i) - 1).^2 + (sum1 - FunctionValue(:,i).^2) - r*r ;
                min1 = min(temp1, min1);
            end;
            min2 = sum((FunctionValue - 1/sqrt(M)).^2, 2) - r*r;
...
function [Output,Boundary,Coding] = P_C3_DTLZ(Operation,Problem,M,Input)
    persistent K;
    Boundary = NaN; Coding = NaN;
    switch Operation
        %????????????
        ...
        case 'value'
            ...
            tcv = FunctionValue;
            for j = 1:M
                tcv(:,j) = FunctionValue(:,j).^2/4 + (sum1 - FunctionValue(:,j).^2) - 1;
            end;
