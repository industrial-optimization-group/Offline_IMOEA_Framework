                        ...
                        def parallel_execute(run, path_to_file):
                            ...
                            if mode == 9:
                                ...
                                if problem_testbench == 'DTLZ':
                                    #path_to_file1 = path_to_file + '/Run_' + str(run) + '_soln_pickle'
                                    ...
                                    results_data=pickle.load(infile)
                                    infile.close()
                                    individual_nds = np.asarray(results_data["individuals_solutions"])
                                    surrogate_objectives_nds = np.asarray(results_data["obj_solutions"])
                                    ...
                                    for i in range(np.shape(individual_nds)[0]):
                                        if i == 0:
                                            actual_objectives_nds = np.asarray(objf(prob, obj, n_vars, individual_nds[i, :])).reshape(1,obj)
                                        else:
