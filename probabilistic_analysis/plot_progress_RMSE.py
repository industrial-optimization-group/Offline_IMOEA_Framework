        ...
        for obj in objectives:
            for n_vars in dims:
                ...
                for algo in emo_algorithm:
                    ...
                    color_map = color_map(np.linspace(0, 1, mode_length+1))
                    color_map = color_map[1:mode_length+1]
                    ax = sns.lineplot(x="Evaluations", y="RMSE",
                                      hue="Approaches", style="Approaches",
                                      markers=["X","s","P","D","d","^","*"], dashes=False, data=hv_df,palette=color_map)
                    ...
                    ax.legend(handles=handles, labels=labels, frameon=False, loc='upper center',
                                                bbox_to_anchor=(0.5, 1.2), ncol=int(mode_length/2)+1)
