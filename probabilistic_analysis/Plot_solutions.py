                ...
                for algo in emo_algorithm:
                    ...
                    for mode, mode_count in zip(modes,range(np.shape(modes)[0])):
                        ...
                        def parallel_execute(run, path_to_file):
                            ...
                            path_to_file0 = path_to_file + '/med_indices.csv'
                            median_index = []
                            with open(path_to_file0, 'r') as f:
                                reader = csv.reader(f)
                                for line in reader: median_index.append(line)
                            run = np.array(median_index, dtype=np.int32)[0][0]
                                ...
                                if problem_testbench == 'DDMOPP':
                                    if mode == 9:
                                        ...
                                        if mode == 0:
                                            ...
                                            actual_objectives_nds = np.array(actual_obj, dtype=np.float32)
                                            ref = [2] * obj
                    ...
                    if obj==2:
                        filename_fig_surrogate = main_directory + '/2d_scatter_' + samp + '_' + algo + '_' + prob + '_' + str(
                        obj) + '_' + str(n_vars) + '.pdf'
                        filename_fig_underlying = main_directory + '/2d_scatter_' + samp + '_' + algo + '_' + prob + '_' + str(
                        obj) + '_' + str(n_vars) + '.pdf'
                    else:
                        filename_fig = main_directory + '/3d_scatter_' + samp + '_' + algo + '_' + prob + '_' + str(
                        obj) + '_' + str(n_vars) + '.pdf'
                    #print(filename_fig)
                    fig.savefig(filename_fig_surrogate, bbox_inches='tight')
                    plt.show()
                    fig.savefig(filename_fig_underlying, bbox_inches='tight')
                    plt.show()
